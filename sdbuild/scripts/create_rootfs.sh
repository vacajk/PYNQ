...
if [ -n "$PYNQ_UBUNTU_REPO" ]; then
  cat - >> $target/postinst2.sh <<EOT
echo "deb http://mirrors.ustc.edu.cn/ubuntu-ports bionic main universe" > /etc/apt/sources.list.d/multistrap-bionic.list
echo "deb-src http://mirrors.ustc.edu.cn/ubuntu-ports bionic main universe" >> /etc/apt/sources.list.d/multistrap-bionic.list
EOT
fi
